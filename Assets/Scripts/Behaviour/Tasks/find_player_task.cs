...
public class find_player_task : Task
{
	...
	public override void Update(float dt, GameObject go){
		...
		Node end_node = cc.nav_path.get_nearest_node (player.transform.position);
		...
		if (end_node != null) {
			...
			var path = cc.nav_path.get_path (start_node, end_node,player.transform.position,0);
